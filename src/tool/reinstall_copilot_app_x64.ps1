...
Write-Host '3. Electron Builder Build (x64)...'
npx electron-builder --win --x64
if ($LASTEXITCODE -ne 0) { Write-Host 'Build finished with expected error. Continuing...' }

Write-Host '4. MakeAppx Packaging (x64)...'
$makeappx = "C:\Program Files (x86)\Windows Kits\10\bin\10.0.26100.0\x64\makeappx.exe"
& $makeappx pack /o /nv /f "dist\__appx-x64\mapping.txt" /p "dist\sidekick-ruka-x64.appx"

Write-Host '5. Signing...'
$signtool = "C:\Program Files (x86)\Windows Kits\10\bin\10.0.26100.0\x64\signtool.exe"
& $signtool sign /f "sidekick-ruka.pfx" /p "password" /fd SHA256 "dist\sidekick-ruka-x64.appx"
...
Add-AppxPackage -Path "dist\sidekick-ruka-x64.appx"
